...
```{r}
olink_anno <- vroom("coding143.tsv")
```
...
``` r
olink_assay <- vroom("olink_assay.dat") %>% 
  mutate(Assay = tolower(Assay))
olink_assay_version <- vroom("olink_assay_version.dat") %>% 
  mutate(Assay = tolower(Assay))
olink_batch_number <- vroom("olink_batch_number.dat")
olink_limit_of_detection <- vroom("olink_limit_of_detection.dat") %>% 
  mutate(Assay = tolower(Assay))
olink_panel_lot_number <- vroom("olink_panel_lot_number.dat")
olink_processing_start_date <- vroom("olink_processing_start_date.dat")
```
...
```{r}
olink_npx_baseline <- vroom("olink_instance_0.csv")
```
...
Let's prepare the assay data first:

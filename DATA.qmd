...
```{r}
#| eval: false
system(glue("dx extract_dataset {project_record_id}.dataset -ddd --delimiter ','"))
```
...
field_entities <- str_c("particpant.", unname(field_ids), collapse = ",")
...
```{r}
pheno_data <- vroom("pheno_data.csv", col_names = field_names)
pheno_data
```

## Extracting data via Spark

If there are more than 30 entities you want to extract, DNANexus recommends connecting to their spark cluster. Here's how we do that in R:
